-- file: 005_castor_business.sql
-- tier: A
-- purpose:
--   * castor_visita_feedback: registro de visita por vendedor + RPC
--     com regras outcome → next_contact_at (negativo/voltar_depois/convertido).
--   * castor_route_log: auditoria de rotas geradas.
--   * castor_haversine_km: utilitário de distância para o roteirizador
--     (nearest-neighbor) executado fora do banco (no n8n) com coordenadas
--     vindas das fontes no Drive.
-- depends: 001, 004
-- reversible: yes
-- IDEMPOTENTE.
...
CREATE TABLE IF NOT EXISTS castor_visita_feedback (
  ...
  cliente_codigo    TEXT NOT NULL,            -- A1_COD || A1_LOJA
  vendedor_user_id  UUID,
  vendedor_codigo   TEXT,                     -- A3_COD
  visited_at        TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  outcome           TEXT NOT NULL CHECK (outcome IN ('negativo','voltar_depois','convertido')),
  custom_days       INT,
...
BEGIN
  ...
  END IF;
  IF p_outcome NOT IN ('negativo','voltar_depois','convertido') THEN
...
-- ============================================================
-- HAVERSINE (utilitário; nearest-neighbor agora vive no n8n)
-- ============================================================
...
INSERT INTO castor_schema_migrations(version) VALUES ('005_business') ON CONFLICT DO NOTHING;
